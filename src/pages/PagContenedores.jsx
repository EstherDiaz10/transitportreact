...
const PagContenedores = () => {
    ...
    if (selectFiltrado !== 'estado') {
        const estadoBooleano = selectFiltrado === 'disponible';
        contenedoresAMostrar = contenedoresAMostrar.filter((c) => c.existe === estadoBooleano);
    }

    if (inputBuscar !== '') {
        contenedoresAMostrar = contenedoresAMostrar.filter((c) => {
            const numSerieEncontrado = c.num_serie.toLowerCase().includes(inputBuscar.toLowerCase());
            const idEncontrada = ("C-" + c.id.toString()).includes(inputBuscar.toLowerCase());
            return numSerieEncontrado || idEncontrada;
        });
    }
    ...
    const columnasContenedores = [
        ...
        },
        {
            ...
            'valor': 'existe',
            'estado': (c) => (
                <div key={c.id} className="flex items-center justify-between gap-3 w-full pr-2">
                    <p className="truncate text-right">{c.existe ? 'Disponible' : 'Ocupado'}</p>
                    <div style={{backgroundColor: colorEstado(c.existe)}} className="w-4 h-4 rounded-full"></div>
                </div>
            )
        }
    ];
    ...
    const ultimoId = (contenedores && contenedores.length > 0 ? Math.max(...contenedores.map(c => c.id)) + 1 : 1);

    return (
        <>
            <h1 className="fixed top-20 text-3xl font-bold text-[#2A5677]">Listado de Contenedores</h1>
            ...
            <div className={`${contenedorSeleccionado ? 'lg:w-[53%]' : 'w-full'}`}>
                <Listado 
                    ...
                    icono={icono}
                />
                <BotonCrear tipo="contenedor" setCrearElemento={setCrearElemento} seleccionado={contenedorSeleccionado}/>
            ...
            <Modal modalAbierto={crearElemento} cerrarModal={() => setCrearElemento(false)}>
                <FormAnyadirContenedor ultimoId={ultimoId} cerrarModal={() => setCrearElemento(false)} />
            </Modal>
        </>
    );
}
