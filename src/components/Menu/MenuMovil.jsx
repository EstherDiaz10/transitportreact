...
const MenuMovil = () => {
    ...
    const rol = user.rol;
    const styles = "w-full flex items-center justify-center font-bold list-none transition-all duration-300";
    const activeStyles = "bg-white text-[#2A5677] p-2 scale-110 rounded-full shadow-md/40";
    const columnasGrid = {
        administrativo: 'grid-cols-4',
        gestor: 'grid-cols-6',
        operario: 'grid-cols-1'
    }
        
    return (
        <div className="lg:hidden">    
            <nav className="fixed inset-x-0 bottom-0 z-50 h-18 bg-[#2A5677] flex items-center justify-center">      
                <ul className={`grid ${columnasGrid[rol]} w-[90%] items-center`}>
                    {rol === 'operario' && (
                        <li>
                            <NavLink to="/misOrdenes" className={({isActive}) => `${styles} ${isActive ? activeStyles : 'text-white'}`}>
                                {({isActive}) => (
                                    <>
                                        ...
                                        {isActive && <span className="pl-5 hidden md:block">Órdenes</span>}
                                    </>
                                )}
                            </NavLink>
                        </li>
                    ...
                    {(rol === 'administrativo' || rol === 'gestor') && (       
                        <>
                            <li>            
                                <NavLink to="/ordenes" className={({isActive}) => `${styles} ${isActive ? activeStyles : 'text-white'}`}>
                                    {({isActive}) => (
                                        <>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="currentColor" viewBox="0 0 256 256"><path d="M229.66,58.34l-32-32a8,8,0,0,0-11.32,0l-96,96A8,8,0,0,0,88,128v32a8,8,0,0,0,8,8h32a8,8,0,0,0,5.66-2.34l96-96A8,8,0,0,0,229.66,58.34ZM124.69,152H104V131.31l64-64L188.69,88ZM200,76.69,179.31,56,192,43.31,212.69,64ZM224,128v80a16,16,0,0,1-16,16H48a16,16,0,0,1-16-16V48A16,16,0,0,1,48,32h80a8,8,0,0,1,0,16H48V208H208V128a8,8,0,0,1,16,0Z"></path></svg>
                                            {isActive && <span className="pl-5 hidden md:block">Órdenes</span>}
                            ...
                            <li>
                                <NavLink to="/gruas" className={({isActive}) => `${styles} ${isActive ? activeStyles : 'text-white'}`}>
                                    {({isActive}) => (
                                        <>
                                            ...
                                            {isActive && <span className="pl-5 hidden md:block">Grúas</span>}
                                        </>
                                    )}
                                </NavLink>
                            </li>        
                                ...
                                <NavLink to="/contenedores" className={({isActive}) => `${styles} ${isActive ? activeStyles : 'text-white'}`}>
                                    {({isActive}) => (
                                        <>
                                            ...
                                            {isActive && <span className="pl-5 hidden md:block">Contenedores</span>}
                                        </>
                                    )}
                                </NavLink>
                            </li>        
                                ...
                                <NavLink to="/buques" className={({isActive}) => `${styles} ${isActive ? activeStyles : 'text-white'}`}>
                                    {({isActive}) => (
                                        <>
                                            ...
                                            {isActive && <span className="pl-5 hidden md:block">Buques</span>}
                                        </>
                                    )}
                                </NavLink>
                            </li>
                                ...
                                <NavLink to="/usuarios" className={({isActive}) => `${styles} ${isActive ? activeStyles : 'text-white'}`}>
                                    {({isActive}) => (
                                        <>
                                            ...
                                            {isActive && <span className="pl-5 hidden md:block">Usuarios</span>}
                                        </>
                                    )}
                                </NavLink>
                            </li>        
                                ...
                                <NavLink to="/patio" className={({isActive}) => `${styles} ${isActive ? activeStyles : 'text-white'}`}>
                                    {({isActive}) => (
                                        <>
                                            ...
                                            {isActive && <span className="pl-5 hidden md:block">Patio</span>}
                                        </>
                                    )}
                                </NavLink>
                            </li>
