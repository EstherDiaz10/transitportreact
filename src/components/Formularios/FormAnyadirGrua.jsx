...
const FormAnyadirGrua = ({ultimoId, cerrarModal, setGruas}) => {
    ...
    const [erroresGrua, setErroresGrua] = useState({});

    const validar = () => {
        let nuevosErrores = {};
        let esValido = true;

        if (!datosNuevaGrua.tipo) {
            nuevosErrores.tipo = "Debes seleccionar un tipo de grúa";
            esValido = false;
        }

        if (!datosNuevaGrua.estado) {
            nuevosErrores.estado = "Debes seleccionar un estado";
            esValido = false;
        }

        if (!datosNuevaGrua.id_zona) {
            nuevosErrores.id_zona = "Debes asignarse a una zona";
            esValido = false;
        }

        setErroresGrua(nuevosErrores);
        return esValido;
    };
    ...
    <div>
        <div className="flex justify-center">
            <div className="w-180 h-180 pt-9 pl-10 pr-10 md:pl-18 md:pr-18 md:pt-13 bg-[#B7D0E1] rounded-[50px]">
                ...
                <form className="text-[#2A5677]" onSubmit={crearNuevaGrua}>
                    ...
                    <div className={lineStyle}>
                        <div className={columnStyle}>
                            ...
                            <input
                                ...
                                id="id_buque"
                                name="id"
                                value={datosNuevaGrua.tipo ? `${datosNuevaGrua.tipo.toUpperCase()}-${ultimoId}` : ""}
                                readOnly
                            />
                        </div>
                        ...
                        <div className={columnStyle}>
                            ...
                            <select
                                ...
                                value={datosNuevaGrua.tipo} 
                                className={`${inputStyle} mt-3 ${erroresGrua.tipo ? "border-2 border-red-500" : ""}`}
                                name="tipo"
                                id="tipo_grua"
                            >
                    ...
                    <div className={`${lineStyle} mt-8`}>
                        ...
                        <div className={columnStyle}>
                            ...
                            <select
                                onChange={handleInput}
                                value={datosNuevaGrua.id_zona} 
                                className={`${inputStyle} mt-3 ${erroresGrua.id_zona ? "border-2 border-red-500" : ""}`}
                                name="id_zona"
                                id="zona_grua"
                            >
                        ...
                        <div className={columnStyle}>
                            ...
                            <select
                                onChange={handleInput}
                                value={datosNuevaGrua.estado} 
                                className={`${inputStyle} mt-3 p-1.5 ${erroresGrua.estado ? "border-2 border-red-500" : ""}`}
                                name="estado"
                                id="estado_grua"
                            >
                    ...
                    <div className='mt-8'>
                        <label className="mb-3 block" htmlFor="operarios_grua">
                            Operarios asignados
                        </label>
                        <MultiSelect
                            options={operarios}
                            value={datosNuevaGrua.operarios}
                            onChange={handleSelectOperarios}
                        />
                    </div>
